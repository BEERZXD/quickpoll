...
On May 9, 2026, OG share metadata was updated. The global description for home, create, and host pages changed to `สร้างโพลสดและโหวตแบบเรียลไทม์!`. Voter page title changed from `bquickpoll | <room code>` to `Room | <room code>`. Voter description changed from poll-title-based (with Worker fetch) to a fixed Thai template: `เข้าร่วมห้อง <room code> และร่วมโหวตโพลแบบเรียลไทม์!`. The server-side poll-title fetch was removed from `page-metadata.ts`, simplifying voter metadata to a synchronous function. Covered with `npm.cmd test` (62 tests passed) and `npx.cmd tsc --noEmit`.
